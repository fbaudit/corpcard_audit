...
- **한국 실무 최적화** — CP949/EUC-KR CSV 자동 감지, `1,234,567`·`₩12,000`·`12000원` 금액 파싱, `2025.03.08`·`2025년 3월 8일`·`20250308` 날짜 파싱, 한국 공휴일(2023–2027, 대체공휴일 포함) 내장
...
| `corpcard_export_report` | 감사보고서(Markdown) / 이상거래(CSV) 파일 저장 |
...
- 카드 데이터는 **전부 로컬에서** 파싱·분석됩니다.
- AI 시나리오 생성 시에만 **컬럼 구조, 통계 요약, 샘플 8행**이 Anthropic API로 전송됩니다. 전체 거래내역은 전송되지 않습니다.
- 민감 데이터라면 카드번호를 마스킹(`5311-****-1101`)한 추출본 사용을 권장합니다.
